#setwd("~/Desktop/Health Work/Cuban")
rm(list=ls())
...
data <- read.csv("J269494/10Plan_data.csv")

file <- "~/Desktop/LandP/Wages/cpi-u-rs.csv"
...
delta.mat$age.group[delta.mat$age >= 18 & delta.mat$age < 35] <- 1
delta.mat$age.group[delta.mat$age >= 35 & delta.mat$age < 50] <- 2
delta.mat$age.group[delta.mat$age >= 50 & delta.mat$age < 65] <- 3
delta.mat <- delta.mat[delta.mat$age.group != 0,]

delta.mat$group <- delta.mat$age.group + 3*(delta.mat$sex - 1)

for (i in 1:6) {
  print(summary(delta.mat$delta[delta.mat$group == i]))
  print(cor(delta.mat$delta[delta.mat$group == i],delta.mat$inc[delta.mat$group == i]))
}
