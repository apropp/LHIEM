...
We can use the program automeris to extract the data mortality and morbidity data from these plots^[ https://apps.automeris.io/wpd/.]. The plots of $l_x$ of the extracted data is shown below. From the values of $l_x$ for mortality and morbidity we compute the mortality and morbidity rates $m_x$ using the **MortalityLaws** package. In finding the $l_x$ and $m_x$ values by age we appliedsome smoothing.  
```{r echo=FALSE}
...
Now, there is an additional issue. The curve in red above gives the $l_x$ of all-cause mortality rate while the blue curve gives the morbidity $l_x$ just for the PCDD. From these $l_x$ values we can compute the hazrad rates $m_x$ for both and take the ratio. However, if we want to compute a ratio we need both the numerator and denominator to respectively reflect the hazard of developing a PCDD and the mortality hazard of PCDDs (instead of the all-cause mortality rate). 
...
p <- ggplot(data = dat.p)+
      ...
      labs(
        x = "Age",
        y = "Transition Prob for acute condition"
      ) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
              labels = trans_format("log10", math_format(10^.x)))+
  ggtitle("Acute Transition Prob")+
   scale_colour_manual(values = c("pink","lightblue","green"))+
  scale_x_continuous(breaks = c(0:10)*10)+
      theme_bw() +
      theme(
        plot.title = element_text(color="black", size=big, face="bold.italic"),
